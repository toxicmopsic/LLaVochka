...
gt_file="${VIDEO_BENCHMARK_ROOT}/benchmarking/Benchmarking_QA/consistency_qa.json"
exp_dir=$LLAVA_MINI_ROOT/exp
output_dir="${exp_dir}/Benchmarking_QA/consistency_qa/${CKPT_NAME}"

for IDX in $(seq 0 $((CHUNKS-1))); do
  CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python3 $LLAVA_MINI_ROOT/llavamini/eval/video/run_inference_benchmark_consistency.py \
