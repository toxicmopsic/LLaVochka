...
from llavamini.mm_utils import tokenizer_image_token, process_images, get_model_name_from_path
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image=split_image(image,n=getattr(self.model_config,'resolution_ratio', 1))
        image_tensor = process_images(image, self.image_processor, self.model_config)

        input_ids = tokenizer_image_token(prompt, self.tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt')

        return input_ids, image_tensor, image[0].size
