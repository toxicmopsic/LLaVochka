...
from llavamini.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN,IMAGE_PLACEHOLDER
...
from llavamini.mm_utils import tokenizer_image_token, process_images, get_model_name_from_path,KeywordsStoppingCriteria
...
PROMPT="""You are a helpful expert in first person view video analysis. Please provide a single-letter answer (A, B, C, D, E) to the following multiple-choice question based on this video.
{question}
Choices:
A: {optionA}
B: {optionB}
...
def get_model_output(model, video_processor, tokenizer, video, qs, args):
    video_frames = load_video(video,num_frm=None)
...
def run_inference(args):
    ...
    for sample in tqdm(gt_contents):
        ...
        question = PROMPT.format(
            ...
            optionA=sample['option 0'],
            optionB=sample['option 1'],
            optionC=sample['option 2'],
            optionD=sample['option 3'],
            optionE=sample['option 4'],
        )
        index += 1
        total+=1
        ...
        for fmt in video_formats:  # Added this line
            ...
            if os.path.exists(temp_path):
                ...
                sample_set['output'] = output
                
                sample_set['pred'] = first_char_as_answer(output)
                if sample_set['pred']==sample_set['answer']:
                    correct+=1
                    sample_set['acc']=1
                else:
                    sample_set['acc']=0
