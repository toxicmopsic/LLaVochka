...
from llavamini.conversation import default_conversation, conv_templates, SeparatorStyle
from llavamini.constants import LOGDIR
from llavamini.utils import build_logger, server_error_msg, violates_moderation, moderation_msg
import hashlib
...
def add_text(state, text, image, image_process_mode,video, request: gr.Request):
...
def http_bot(state, model_selector, temperature, top_p, max_new_tokens, request: gr.Request, template_name=None):
    ...
    try:
        # Stream output
        response = requests.post(worker_addr + "/worker_generate_stream", headers=headers, json=pload, stream=True, timeout=100000)
...
title_markdown = """
# **LLaVA-Mini**: Efficient Image and Video Large Multimodal Models with One Vision Token  [[Paper]()] [[Code](https://github.com/ictnlp/LLaVA-Mini)] [[Model](https://huggingface.co/ICTNLP/llava-mini-llama-3.1-8b)] 
...
def build_demo(embed_mode):
    ...
    with gr.Blocks(title="LLaVA-Mini", theme=gr.themes.Default(), css=block_css) as demo:
        state = gr.State()
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                videobox = gr.Video(sources=['upload', 'webcam'])
            ...
            with gr.Column(scale=9):
                chatbot = gr.Chatbot(elem_id="chatbot", label="LLaVA-Mini Chatbot", height=550)
                with gr.Row():
                    with gr.Column(scale=9):
                        textbox.render()
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=4.5):
                        gr.Examples(
                            examples=[
                                [f"{cur_dir}/examples/extreme_ironing.jpg", "这幅图像有何不寻常之处？"],
                                [f"{cur_dir}/examples/baby_cake.png", "What's the text on the cake?"],
                            ],
                    ...
                    with gr.Column(scale=4.5):
                        gr.Examples(
                            examples=[
                                [f"{cur_dir}/examples/safari.mp4", "描述这个视频？"],
                                [f"{cur_dir}/examples/fifa.mp4", "What happened in this video?"],
                            ],
        ...
        clear_btn.click(clear_history, None, [state, chatbot, textbox, imagebox,videobox] + btn_list, queue=False)

        textbox.submit(add_text, [state, textbox, imagebox, image_process_mode,videobox], [state, chatbot, textbox, imagebox,videobox] + btn_list, queue=False).then(
            http_bot, [state, model_selector, temperature, top_p, max_output_tokens], [state, chatbot] + btn_list
        )

        submit_btn.click(add_text, [state, textbox, imagebox, image_process_mode,videobox], [state, chatbot, textbox, imagebox,videobox] + btn_list, queue=False).then(
            http_bot, [state, model_selector, temperature, top_p, max_output_tokens], [state, chatbot] + btn_list
        )
